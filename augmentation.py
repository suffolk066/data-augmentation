...
class Augmentationer():
    ...
    def set_aug_type(self, aug_type:str) -> A.Compose:
        ...
        return A.Compose([self.aug_func[aug_type.upper()]], bbox_params=A.BboxParams(format='yolo'))
    ...
    def run(self, aug_type:str, data:dict) -> None:
        ...
        with progress_bar as p:
            ...
            for image, txt in data.items():                  
                ...
                save_file_name = os.path.join(self.SAVE_DIR, save_prefix + original_file_name)
    ...
    def main(self, aug_type:str) -> None:
        data = self.get_data()
        self.run(aug_type ,data)
        
@click.command()
@click.option("-t", required=True, type=click.Choice(['vflip', 'hflip', 'rotate', 'random']), help='Set augmentation type')
def execute(t:str):
    Augmentationer().main(t)
